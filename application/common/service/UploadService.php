...
use app\common\service\storage\OSS;
use app\common\service\storage\Qiniu;
...
//        self::$uploadStatus->status = true;
     ...
     * @return mixed
     */
    public static function image($fileName, $path = null)
    {
        $uploadType = config('upload.type');
        $uploadType = $uploadType ? $uploadType : 1;
        $info = [];
        ...
        switch ($uploadType) {
            ...
            case 'oss' :
                ...
                $serverImageInfo['info']['url'] = setHttpType($serverImageInfo['info']['url']);
                $info['dir'] = $serverImageInfo['info']['url'];
                $info['thumb_path'] = $serverImageInfo['info']['url'];
                $headerArray = get_headers(setHttpType($serverImageInfo['info']['url'], 1), true);
